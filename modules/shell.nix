{pkgs, ...}:
{
 environment.shellAliases = {
  ...
  rebuild-reboot = "sudo nixos-rebuild boot --flake $HOME/.config/nixos-config#$(hostname) && read -p 'Nixos rebuild successful. Reboot now? (y/N) ' -n 1 -r && echo && [[ $REPLY =~ ^[yY]$ ]] && sudo reboot";
